...
df_territoires["CODEID"] = df_territoires["CODEID"].astype(int)

# Calcul des intersections entre espaces verts et territoires
territory_superficie = {territory["properties"]["CODEID"]: 0 for territory in territoires_MTL_Clean_geojson_data["features"]}
territory_shapes = {territory["properties"]["CODEID"]: shape(territory["geometry"]) for territory in territoires_MTL_Clean_geojson_data["features"]}
...
territory_parc_count = {territory["properties"]["CODEID"]: 0 for territory in territoires_MTL_Clean_geojson_data["features"]}
...
def carte_espaces_verts(_zoom, _center, _geojson_data):
...
espace_verts_map = carte_espaces_verts(10, {"lat": 45.55, "lon": -73.75}, espace_vert_geojson_data)

# Création de la carte des territoires
territoires_map = px.choropleth_map(
    df_territoires,
...
def update_map_and_hover_info(hoverData):
    ...
    try:
        ...
        territory_shape = shape(selected_territory["geometry"])
        ...
        updated_map = carte_espaces_verts(12, center, filtered_geojson_data)
