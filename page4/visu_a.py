...
fig.add_trace(
    go.Choroplethmapbox(
        ...
        marker=dict(line=dict(width=1, color="white"), opacity=1.0),  # Solid white outline
        hoverinfo="skip"
    )
)
...
# Ensure the plot is displayed with interactive config
fig.show(config={
    'scrollZoom': True,
...
def display_jardin_count(hover_data):
    ...
    try:
        ...
        return f"Number of jardins in {arrondissement}: {jardin_count}"
